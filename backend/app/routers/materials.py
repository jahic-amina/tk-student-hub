...
from app.models.materials import Material, MaterialsResponse, MaterialDetailResponse, Rating, Comment, Subject, RatingCreate, CommentResponse, CommentCreate, Bookmark
...
def delete_material(id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    material = db.query(Material).filter(Material.id == id).first()
    if not material:
...
def rate_material(id: int, rating_data: RatingCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    material = db.exec(select(Material).where(Material.id == id)).first()
    if not material:
        raise HTTPException(status_code=404, detail="Materijal nije pronađen.")

    existing = db.exec(select(Rating).where(Rating.material_id == id, Rating.user_id == current_user.id)).first()
    if existing:
...
def update_rating(id: int, rating_data: RatingCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    existing = db.exec(select(Rating).where(Rating.material_id == id, Rating.user_id == current_user.id)).first()
    if not existing:
...
def create_comment(material_id: int, comment_data: CommentCreate, session: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    material = session.get(Material, material_id)
    if not material or material.status == "deleted":
        raise HTTPException(status_code=404, detail="Materijal nije pronađen.")
...
def delete_comment(material_id: int, comment_id: int, session: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def toggle_bookmark(material_id: int, session: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.role == "admin":
        raise HTTPException(status_code=403, detail="Admini ne mogu bookmarkovati materijale.")

    statement = select(Bookmark).where(Bookmark.user_id == current_user.id, Bookmark.material_id == material_id)
