...
// Izvlačenje medalja iz strukture koju šalje backend (get_user_forum_identity)
const authorMedals = computed(() => {
  return props.topic?.author?.medals || [];
});

// Prve tri istaknute medalje koje stoje u glavnoj liniji kartice
const featuredMedals = computed(() => {
  return authorMedals.value.slice(0, 3);
});

// Sve preostale medalje (od 4. pa nadalje) koje idu u dropdown
...
<template>
  ...
  >
    ...
    <div class="flex items-center flex-wrap gap-2 text-xs text-slate-500 dark:text-slate-400 mb-4 bg-slate-50 dark:bg-slate-700 p-2 rounded-lg w-fit">
      ...
      <span class="font-bold uppercase text-[10px] px-1.5 py-0.5 rounded border" :class="getRoleBadgeClass(topic.author?.role)">
        {{ topic.author?.role || 'Student' }}
      </span>
      
      <span class="text-[10px] font-medium text-slate-400 dark:text-slate-500">
        Lvl {{ topic.author?.level || 1 }}
      </span>
      ...
      <div v-if="authorMedals.length > 0" class="flex items-center gap-1 ml-2 border-l pl-2 border-slate-200 dark:border-slate-600 relative medals-dropdown-container">
        ...
        <span
          ...
          :title="parseMedal(rawMedal).tooltip"
        >
          {{ parseMedal(rawMedal).icon }}
        </span>

        <button 
          v-if="remainingMedals.length > 0"
          @click.stop="showAllMedalsDropdown = !showAllMedalsDropdown"
        ...
        >
          ...
          <div class="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto">
            <span 
              v-for="rawMedal in remainingMedals" 
              :key="rawMedal.code || rawMedal.id"
        ...
        <div class="flex items-center gap-2">
          <button @click="copyToClipboard" class="flex flex-col items-center gap-1 p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors bg-transparent border-none" title="Kopiraj link">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 00-5.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
            </svg>
